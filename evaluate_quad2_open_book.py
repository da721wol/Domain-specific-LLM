...
# Load dataset
dataset = load_dataset("deepset/germandpr", split="test")

# Remove unnecessary columns
dataset = dataset.map(remove_columns=["negative_ctxs", "hard_negative_ctxs"])
...
def evaluate_open_book(model, tokenizer, name, dataset=dataset, rouge=rouge):
  ...
  def generate_answer(batch):
    inputs = ["Beantworte mit folgendem Kontext die Frage: " + row[0] + " " + row[1]["text"][0] for row in zip(batch['question'], batch['positive_ctxs'])]
  ...
  with open('../quad2_open_book_test.jsonl', 'a') as f:
